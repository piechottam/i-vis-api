...
from sqlalchemy import tuple_, or_, func, literal
...
def parse_core_types_args(
    ...
    core_type_args: MutableMapping[CoreType, MutableMapping[str, Any]] = {}
    key2core_type = {}
    for core_type in core_types:
        if core_type.harm_meta.target == "hgvs":
            continue  # FIXME remove
        for key in core_type.harm_meta.fields.keys():
            key2core_type[key] = core_type
    for key, value in query_args.items():
        if key in key2core_type:
            core_type_args.setdefault(key2core_type[key], {})[key] = value
    return core_type_args
...
def parts_by_non_hgvs(
    ...
    db_query: Optional[db.Query] = None,
) -> db.Query:
        ...
        for core_type_, key2values in core_type2kwargs.items():
            ...
            if model != harmonized_model:
                db_query = db_query.join(
                    model, harmonized_model.raw_data_id == model.raw_data_id
                )
            for key, value in key2values.items():
                if isinstance(value, list):
                    db_query = db_query.filter(getattr(model, key).in_(value))
...
def _get_match_obs(
    hgvs_harm_model: Type["HarmonizedData"], query: DataFrame
...
def _process_hgvs_result_match(
    hgvs_result_match: int, hgvs_harm_model: Type["HarmonizedData"], query: DataFrame
) -> Any:
    harm_model_cols = []
    query_cols = []
...
def parts_by_hgvs(
    ...
    hgvs_harm_model = etl.core_type2model[hgvs_core_type]
...
def raw_data(raw_model: "RawData", db_query: db.Query) -> db.Query:
    return db.session.query(raw_data).filter(raw_model.id.in_(db_query))
